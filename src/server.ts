...
async function handleIncomingMessages(id: string, sock: WASocket, messages: any[], type: string) {
  ...
  for (const msg of messages) {
    try {
      ...
      if (!isTrigger && now - lastReplyTime <= fourHours) continue;
      ...
      logger.error(
        { error: error?.message || error, sessionId: id },
...
app.get("/session/:sessionId/qr", authMiddleware, (req, res) => {
  ...
  if (!session.qrDataUrl) {
    res.status(404).json({
      ...
      status: session.status,
      retryable: session.status === "starting" || session.status === "disconnected",
    });
    return;
  }
  ...
  logger.info(
    {
      ...
      vercelApiConfigured: Boolean(VERCEL_API_URL),
    },
