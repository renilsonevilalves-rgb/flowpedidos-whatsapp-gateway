...
import { Browsers, DisconnectReason, fetchLatestWaWebVersion, makeWASocket, useMultiFileAuthState, } from "@whiskeysockets/baileys";
...
async function clearAuthState(id) {
    try {
...
function scheduleReconnect(id, fresh = false, delayMs = 1500) {
    const session = getOrCreateSession(id);
    if (manualLogouts.has(id) || session.reconnectTimer)
        return;
...
async function waitForQrOrConnected(id, timeoutMs = 10000) {
    const session = getOrCreateSession(id);
    const deadline = Date.now() + timeoutMs;
    const pollIntervalMs = 50; // OPTIMIZED: reduced from 200ms to 50ms for faster QR detection
    while (Date.now() < deadline) {
        if (session.status === "qr" || session.status === "connected" || session.status === "error")
            return session.status;
        await new Promise((resolve) => setTimeout(resolve, pollIntervalMs));
    }
    return session.status;
}
async function generateQrDataUrlAsync(qrRaw, sessionId) {
    try {
...
async function fetchStoreInfo(sessionId) {
    if (!VERCEL_API_URL)
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(`Vercel store-info returned HTTP ${response.status}: ${data?.error || "unknown error"}`);
        return data;
    }
    finally {
        clearTimeout(timer);
...
function isTrackOrderTrigger(text) {
...
async function fetchOrderStatus(sessionId, phone, orderNumber = '') {
    if (!VERCEL_API_URL)
        throw new Error("VERCEL_API_URL is not configured on the gateway");
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 8000);
    try {
        ...
        const data = await response.json().catch(() => ({}));
        if (!response.ok)
            throw new Error(data?.error || `Order-status returned HTTP ${response.status}`);
        return data;
    }
    finally {
        clearTimeout(timer);
...
function isAutoReplyTrigger(text) {
    ...
    if (greetingOnly.test(normalized))
        return true;
...
async function resolveCustomerPhone(sock, msg, remoteJid) {
    const candidates = [
...
async function handleIncomingMessages(id, sock, messages, type) {
    if (type !== "notify")
        return;
            ...
            if (isTrackingTrigger) {
                try {
                    const customerJid = [
                        ...
                        msg.key.participantPn,
                        remoteJid,
                    ].find((value) => typeof value === "string" && /@s\.whatsapp\.net$/.test(value)) ||
                        msg.key.remoteJidAlt ||
                        msg.key.senderPn ||
                        msg.key.participantPn ||
                        remoteJid;
...
async function connectSession(id, forceFresh = false) {
    const session = getOrCreateSession(id);
    if (session.starting)
...
async function startSessionWithRecovery(id) {
    const session = getOrCreateSession(id);
    if (session.status === "connected" && session.sock)
        return;
...
app.post("/session/:sessionId/send", authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const recipient = recipients.find((item) => item?.exists && item?.jid);
        if (!recipient?.jid) {
...
app.get("/session/:sessionId/qr", authMiddleware, (req, res) => {
    ...
    if (!session.qrDataUrl) {
        res.status(404).json({ ok: false, error: "QR code not available", status: session.status });
        return;
    }
